...
<head>
    ...
    <title>{{ ucfirst($section->type) }} - IELTS Mock Test by UKG</title>
...
<body>
    <div class="test-topbar">
        <div class="d-flex align-items-center">
            <img src="{{ asset('images/logo.png') }}" class="logo" alt="UKG">
            <span class="section-name">{{ $section->title }}</span>
        </div>
        <div class="d-flex align-items-center gap-3">
            <div class="timer-box" id="timer">{{ sprintf('%02d', $section->duration_minutes) }}:00</div>
    ...
    <div class="test-body">
        ...
        <div class="panel-left" id="questionsPanel">
            @include('test.partials.questions', ['questions' => $section->questions, 'existingAnswers' => $existingAnswers])
        </div>
        <div class="panel-right">
            <div class="audio-container">
                ...
                <audio id="listeningAudio" controls controlsList="nodownload"><source src="{{ asset($section->content) }}" type="audio/mpeg"></audio>
            </div>
        </div>
        ...
        <div class="panel-right">
            ...
            <div class="passage-content" id="passageContent">{!! nl2br(e($section->content)) !!}</div>
        </div>
        @elseif($section->type === 'writing')
        <div class="panel-full">
            ...
            <div class="tab-content">
                <div class="tab-pane fade show active" id="task1Tab">
                    <div class="card mb-3 border-primary"><div class="card-body"><h6 class="text-primary">Writing Task 1</h6><p>{{ $section->content }}</p><small class="text-muted">Spend about 20 minutes. Write at least 150 words.</small></div></div>
                    <textarea class="writing-area" id="task1">{{ $writingSub->task1_response ?? '' }}</textarea>
                    <div class="word-count">Word count: <strong id="wc1">{{ $writingSub->task1_word_count ?? 0 }}</strong></div>
                </div>
                <div class="tab-pane fade" id="task2Tab">
                    <div class="card mb-3 border-primary"><div class="card-body"><h6 class="text-primary">Writing Task 2</h6><p>{{ $section->content_extra }}</p><small class="text-muted">Spend about 40 minutes. Write at least 250 words.</small></div></div>
                    <textarea class="writing-area" id="task2">{{ $writingSub->task2_response ?? '' }}</textarea>
                    <div class="word-count">Word count: <strong id="wc2">{{ $writingSub->task2_word_count ?? 0 }}</strong></div>
                </div>
            </div>
        </div>
        @elseif($section->type === 'speaking')
    ...
    <div class="q-nav">
        <div class="q-nav-numbers" id="qNav">
            @if(in_array($section->type, ['listening','reading']))
            @foreach($section->questions as $q)
            <div class="q-nav-num {{ isset($existingAnswers[$q->id]) ? 'answered' : '' }}" data-q="{{ $q->id }}" onclick="scrollToQuestion({{ $q->id }})">{{ $q->question_number }}</div>
            @endforeach
            @endif
        </div>
        <div class="d-flex gap-2 align-items-center">
            @foreach($allSections as $s)
                @if($s->id === $section->id)<span class="badge bg-primary px-3 py-2">{{ ucfirst($s->type) }}</span>
                @elseif($s->order < $section->order)<span class="badge bg-success px-3 py-2"><i class="bi bi-check"></i> {{ ucfirst($s->type) }}</span>
                @else<span class="badge bg-secondary px-3 py-2">{{ ucfirst($s->type) }}</span>@endif
            @endforeach
            <form method="POST" action="{{ route('test.submit-section', ['attempt'=>$attempt->id,'section'=>$section->id]) }}" id="submitForm">@csrf
    ...
    <script>
    const CSRF=document.querySelector('meta[name="csrf-token"]').content,ATTEMPT_ID={{ $attempt->id }},SECTION_ID={{ $section->id }};
    let timeLeft={{ $section->duration_minutes }}*60;const timerEl=document.getElementById('timer');
    ...
    function saveAnswer(qid,val){showSave('saving');fetch('{{ route("test.save-answer") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF},body:JSON.stringify({attempt_id:ATTEMPT_ID,question_id:qid,answer_text:val})}).then(()=>{showSave('saved');updateNavDot(qid,val)})}
